...
const ChattBox:React.FC<ChattBoxProps> = ({serverId, channelId, userId, server, member}) => {
    return(
        <div className="h-full w-full bg-stone-900 bg-opacity-50 flex flex-col">
            ...
            <ChattMessages userId={userId} chatId={channelId} apiUrl={"/api/messages/"} socketUrl={"/api/socket/messages"} socketQuery={{
                    channelId: channelId,
                    serverId: serverId}}
            paramValue={channelId} channelId={channelId} member={member}/>
